...
function renderMovies(movieData) {
  ...
  movieData.forEach((movie) => {
    ...
    movieCard.innerHTML = `
      ...
      isMovieFavorite(movie.id) ? 'class="favorite"' : ""
    }>${isMovieFavorite(movie.id) ? "Remove" : "Save"}</button>
    `;
...
async function searchMovies() {
  ...
  const mov = moviess.results.filter((movie) =>
...
function updatePaginationButtons() {
  ...
  document.getElementById(
    "currentPage"
  ).textContent = `Current Page: ${currentPage}`;
}
